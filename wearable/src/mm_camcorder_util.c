...
// AUDIO TAG - VOICE RECORDER
gboolean _mmcamcorder_write_saut(FILE *f)
{
	gint64 current_pos, pos;
	gchar *str = NULL;
	...
	if(!write_to_32(f, 0)) // saut atomic size
		return FALSE;

	str = str_to_utf8("saut");	// fourcc
	if(str != NULL){
		FPUTS_CHECK(str, f);
		SAFE_FREE(str);
	}
	...
	if(!write_to_32(f, 1))	// add field - voice recorder field
		return FALSE;
...
gboolean _mmcamcorder_write_smta(FILE *f)
{
	gint64 current_pos, pos;
	...
	if(!write_to_32(f, 0)) // smta atomic size
		return FALSE;

	if(!write_tag(f, "smta")) // fourcc smta
		return FALSE;

	FPUTC_CHECK(0, f);		// version 1byte 0

	if(!write_to_24(f, 0))	// flags 3byte 0
		return FALSE;

	if(!_mmcamcorder_write_saut(f))
		return FALSE;
...
gboolean _mmcamcorder_write_udta_m4a(FILE *f)
{
	gint64 current_pos, pos;
...
void _mmcamcorder_write_Longitude(FILE *f,int degreex10000) {
	bool isNegative = (degreex10000 < 0);
	char sign = isNegative? '-': '+';
...
gboolean _mmcamcorder_write_udta(FILE *f, _MMCamcorderLocationInfo info, _MMCamcorderLocationInfo geotag)
{
	gint64 current_pos, pos;
...
void _mmcamcorder_remove_data_probe(MMHandleType handle, _MMCamcorderHandlerCategory category)
{
	...
	if (!hcamcorder->data_probes) {
		_mmcam_dbg_warn("list for data probe is NULL");
		return;
	}
...
unsigned int _mmcamcorder_get_fourcc(int pixtype, int codectype, int use_zero_copy_format)
{
	...
	case MM_PIXEL_FORMAT_RGBA:
		if (use_zero_copy_format) {
			fourcc = GST_MAKE_FOURCC ('S', 'R', '3', '2');
		} else {
                                   ...
                                   unsigned char **dst_data, unsigned int *dst_width, unsigned int *dst_height, unsigned int *dst_length)
{
	...
	case MM_PIXEL_FORMAT_I420:
		...
		break;
	case MM_PIXEL_FORMAT_422P:
		input_format = MM_UTIL_IMG_FMT_YUV422;
		break;
	case MM_PIXEL_FORMAT_YV12:
	...
	case MM_PIXEL_FORMAT_RGB888:
		...
		break;
	case MM_PIXEL_FORMAT_RGBA:
		input_format = MM_UTIL_IMG_FMT_BGRA8888;
		break;
	default:
	...
	if (mm_ret != MM_ERROR_NONE) {
		_mmcam_dbg_err("mm_util_resize_image failed 0x%x", ret);
		free(dst_tmp_data);
		return FALSE;
	}
...
gboolean _mmcamcorder_convert_YUYVto422P(unsigned char *src_data, unsigned char **dst_data,
                                         unsigned int width, unsigned int height, unsigned int length)
{
	unsigned char *yuv_422p_data = NULL;
	unsigned char *yuv_422p_y = NULL;
	unsigned char *yuv_422p_u = NULL;
	unsigned char *yuv_422p_v = NULL;

	if (src_data == NULL || dst_data == NULL || length <= 0) {
		_mmcam_dbg_err("invalid argument %p %p %d", src_data, dst_data, length);
		return FALSE;
	}

	_mmcam_dbg_log("start - size [%dx%d]", width, height);

	yuv_422p_data = (unsigned char *)malloc(length);
	if (yuv_422p_data) {
		int i = 0;
		int index_y = 0;
		int index_uv = 0;

		yuv_422p_y = yuv_422p_data;
		yuv_422p_u = yuv_422p_data + (length>>1);
		yuv_422p_v = yuv_422p_u + (length>>2);

		for (i = 0, index_y = 0, index_uv = 0 ; i < length ; ) {
			yuv_422p_y[index_y++]  = src_data[i++];
			yuv_422p_u[index_uv]   = src_data[i++];
			yuv_422p_y[index_y++]  = src_data[i++];
			yuv_422p_v[index_uv++] = src_data[i++];
		}

		*dst_data = yuv_422p_data;

		_mmcam_dbg_log("done - %p", yuv_422p_data);

		return TRUE;
	} else {
		_mmcam_dbg_err("failed to alloc %d", length);
		return FALSE;
	}
}


gboolean _mmcamcorder_convert_422PtoYUYV(unsigned char *src_data, unsigned char **dst_data,
                                         unsigned int width, unsigned int height, unsigned int length)
{
	int i = 0;
	int base_y = 0;
	...
	_mmcam_dbg_log("start color convert");

	yuyv_data = (unsigned char *)malloc(length);
	if (yuyv_data) {
		base_y = 0;
		base_u = width * height;
		base_v = base_u + (base_u>>1);

		for (i = 0 ; i < length ; ) {
			yuyv_data[i++] = src_data[base_y++];
			yuyv_data[i++] = src_data[base_u++];
			yuyv_data[i++] = src_data[base_y++];
			yuyv_data[i++] = src_data[base_v++];
		}

		*dst_data = yuyv_data;

		_mmcam_dbg_log("done - %p", yuyv_data);
