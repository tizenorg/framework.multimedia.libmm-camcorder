...
| MACRO DEFINITIONS									|
========================================================================================*/
#define _mmcam_dbg_verb(fmt, args...)  mmf_debug(MMF_DEBUG_VERBOSE, " "fmt"\n", ##args);
#define _mmcam_dbg_log(fmt, args...)   mmf_debug(MMF_DEBUG_LOG," "fmt"\n", ##args);
#define _mmcam_dbg_warn(fmt, args...)  mmf_debug(MMF_DEBUG_WARNING," "fmt"\n", ##args);
#define _mmcam_dbg_err(fmt, args...)   mmf_debug(MMF_DEBUG_ERROR," "fmt"\n", ##args);
#define _mmcam_dbg_crit(fmt, args...)  mmf_debug(MMF_DEBUG_CRITICAL," "fmt"\n", ##args);
...
typedef struct mmf_camcorder {
	...
	mm_camcorder_video_frame_render_error_callback render_error_cb; /* video frame render error callback */
	void* render_error_cb_user_param;                               /* video frame render error callback user parameter */
...
#ifdef USE_ASM_SUBEVENT
	int asm_register;                                       /**< Use ASM internally */
	...
	int try_set_hybrid_mode;                                /**< try to set hybrid mode */
	int is_set_hybrid_mode;                                 /**< whether set hybrid mode before set videosrc caps */
	int dual_camera_buffer_0;                               /**< flag for camera buffer is comming or not */
	int dual_camera_buffer_1;                               /**< flag for camera buffer is comming or not */
	...
	int dual_buffer_count;                                  /**< support Zero Shutter Lag capture */
	char *model_name;                                       /**< model name from system info */
	char *software_version;                                 /**< software_version from system info */
	pthread_mutex_t dual_buffer_lock;                       /**< mutex for dual camera */
	pthread_cond_t dual_buffer_cond;                        /**< cond for dual camera */
	void *jpeg_data[2];                                     /**< jpeg_data for dual camera capture */
	int jpeg_size[2];                                       /**< jpeg_size for dual camera capture */
	int send_capture_sound_msg;                             /**< flag for capture sound message */
...
/**
 *	This function is to set callback for video frame render error.
 *
 *	@param[in]	hcamcorder	Specifies the camcorder  handle
 *	@param[in]	callback	Specifies the function pointer of callback function
 *	@param[in]	user_data	Specifies the user poiner for passing to callback function
 ...
 *	@see		mmcamcorder_error_type
 */
int _mmcamcorder_set_video_frame_render_error_callback(MMHandleType handle,
                                                       mm_camcorder_video_frame_render_error_callback callback,
                                                       void *user_data);
