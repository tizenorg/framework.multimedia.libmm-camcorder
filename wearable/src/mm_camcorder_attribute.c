...
mm_cam_attr_construct_info cam_attrs_const_info[] ={
	...
	{
		MM_CAM_CAMERA_HYBRID_MODE,
		"camera-hybrid-mode",
		MMF_VALUE_TYPE_INT,
		MM_ATTRS_FLAG_RW,
		{(void*)MM_CAMCORDER_HYBRID_OFF},
		MM_ATTRS_VALID_TYPE_INT_RANGE,
		MM_CAMCORDER_HYBRID_OFF,
		MM_CAMCORDER_HYBRID_ON,
		_mmcamcorder_commit_camera_hybrid_mode,
	},
	{
	...
	{
		MM_CAM_CAPTURE_SOUND,
		"capture-sound",
		MMF_VALUE_TYPE_INT,
		MM_ATTRS_FLAG_RW,
		{(void*)MM_CAM_CAPTURE_SOUND_DEFAULT},
		MM_ATTRS_VALID_TYPE_INT_RANGE,
		MM_CAM_CAPTURE_SOUND_DEFAULT,
		MM_CAM_CAPTURE_SOUND_EXT_02,
		NULL,
	},
	{
		MM_CAM_VIDEO_WIDTH,
	...
	},
	{
		...
		FALSE,
		TRUE,
		_mmcamcorder_commit_auto_exposure_lock,
	},
	//124
	{
		MM_CAM_AUTO_WHITE_BALANCE_LOCK,
		...
		FALSE,
		TRUE,
		_mmcamcorder_commit_auto_white_balance_lock,
	}
};
...
bool _mmcamcorder_commit_camera_hybrid_mode(MMHandleType handle, int attr_idx, const mmf_value_t *value)
{
	mmf_camcorder_t *hcamcorder = MMF_CAMCORDER(handle);

	mmf_return_val_if_fail(handle, FALSE);

	hcamcorder->try_set_hybrid_mode = value->value.i_val;

	_mmcam_dbg_log("try to set hybrid mode %d", hcamcorder->try_set_hybrid_mode);

	if (hcamcorder->support_zsl_capture) {
		hcamcorder->is_set_hybrid_mode = TRUE;
		_mmcam_dbg_log("Hybrid mode will be applied when preview starts");
	} else {
		if (value->value.i_val) {
			_mmcam_dbg_warn("can NOT support hybrid ON");
			return FALSE;
		} else {
			_mmcam_dbg_log("hybrid mode not supported format, but FALSE could be set");
		}
	}
...
static bool __mmcamcorder_set_capture_resolution(MMHandleType handle, int width, int height)
{
	...
	if (sc->element && sc->element[_MMCAMCORDER_VIDEOSRC_SRC].gst) {
		if (current_state == MM_CAMCORDER_STATE_PREPARE &&
		    hcamcorder->is_set_hybrid_mode) {
			_mmcam_dbg_log("Hybrid mode was set, capture size will be applied next preview starts");
		} else if (current_state <= MM_CAMCORDER_STATE_PREPARE) {
...
__mmcamcorder_check_valid_pair( MMHandleType handle, char **err_attr_name, const char *attribute_name, va_list var_args )
{
	...
	int  i = 0, j = 0;
	char *name = NULL;
	char *check_pair_name[CHECK_COUNT][3] = {
...
bool _mmcamcorder_commit_auto_exposure_lock(MMHandleType handle, int attr_idx, const mmf_value_t *value)
{
	mmf_camcorder_t *hcamcorder = MMF_CAMCORDER(handle);
	_MMCamcorderSubContext *sc = NULL;
	int current_state = MM_CAMCORDER_STATE_NULL;
	...
	if (sc->element && sc->element[_MMCAMCORDER_VIDEOSRC_SRC].gst) {
		...
		_mmcam_dbg_warn("set AE lock - %d", value->value.i_val);

		control = GST_CAMERA_CONTROL(sc->element[_MMCAMCORDER_VIDEOSRC_SRC].gst);
		gst_camera_control_set_ae_lock(control, value->value.i_val);
	} else {
		_mmcam_dbg_err("element is not existed");
		return FALSE;
	}
...
bool _mmcamcorder_commit_auto_white_balance_lock(MMHandleType handle, int attr_idx, const mmf_value_t *value)
{
	mmf_camcorder_t *hcamcorder = MMF_CAMCORDER(handle);
	_MMCamcorderSubContext *sc = NULL;
	int current_state = MM_CAMCORDER_STATE_NULL;
	...
	if (sc->element && sc->element[_MMCAMCORDER_VIDEOSRC_SRC].gst) {
		...
		_mmcam_dbg_warn("set AWB lock - %d", value->value.i_val);

		control = GST_CAMERA_CONTROL(sc->element[_MMCAMCORDER_VIDEOSRC_SRC].gst);
		gst_camera_control_set_awb_lock(control, value->value.i_val);
	} else {
		_mmcam_dbg_err("element is not existed");
		return FALSE;
	}
