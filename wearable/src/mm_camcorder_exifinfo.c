...
mm_exif_write_exif_jpeg_to_memory (void **mem, unsigned int *length, mm_exif_info_t *info,  void *jpeg, unsigned int jpeg_len)
{
	...
	if (mm_exif_load_exif_info(&test_exif_info, jpeg, jpeg_len) == MM_ERROR_NONE) {
		if (test_exif_info) {
			...
			mmf_debug(MMF_DEBUG_ERROR, "test_exif_info is NULL\n");
		}
	} else {
		mmf_debug(MMF_DEBUG_INFO, "no EXIF in JPEG\n");
	}
	...
	mmf_debug(MMF_DEBUG_LOG,"[%05d][%s] JPEG+EXIF Copy DONE(original:%d, offset:%d, copied:%d)\n",
	                        __LINE__, __func__, jpeg_len, jpeg_offset, jpeg_len - jpeg_offset);
	...
	if (loader) {
		...
		if (s > 0) {
			...
			if (x) {
				...
				x->size = s;
				*info = x;
				mmf_debug(MMF_DEBUG_INFO, "load EXIF : data %p, size %d", x->data, x->size);
			} else {
				mmf_debug(MMF_DEBUG_ERROR, "mm_exif_info_t malloc failed");
			}
		} else {
			mmf_debug(MMF_DEBUG_ERROR, "exif_loader_get_buf failed");
		}
		...
		mmf_debug(MMF_DEBUG_ERROR, "exif_loader_new failed");
	}
