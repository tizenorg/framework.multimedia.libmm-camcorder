...
---------------------------------------------------------------------------------------*/
#define THUMBNAIL_WIDTH         160 // 320  -> 160 thumbanil size issue
...
#define WRITE_EXIF_MAKER_INFO
#define WRITE_EXIF_DEBUGGING_INFO
#ifdef WRITE_EXIF_DEBUGGING_INFO
#define EXIF_DEBUGGING_INFO_LENGTH 6400
#endif /* WRITE_EXIF_DEBUGGING_INFO */
...
int _mmcamcorder_image_cmd_capture(MMHandleType handle)
{
	...
	ret = mm_camcorder_get_attributes(handle, &err_name,
	                            ...
	                            MMCAM_CAPTURE_SOUND, &info->capture_sound,
	                            NULL);
	if (err_name) {
	...
	if (!sc->bencbin_capture) {
		...
		if (cap_format == MM_PIXEL_FORMAT_ENCODED) {
			if ((sc->SensorEncodedCapture && info->type == _MMCamcorder_SINGLE_SHOT) ||
				...
				cap_fourcc = _mmcamcorder_get_fourcc(MM_PIXEL_FORMAT_YUYV, image_encoder, hcamcorder->use_zero_copy_format);
	...
	if ((info->type == _MMCamcorder_SINGLE_SHOT &&
		...
		if ((current_state < MM_CAMCORDER_STATE_RECORDING ||
			...
			switch (info->capture_sound) {
			case MM_CAM_CAPTURE_SOUND_EXT_01:
				_mmcamcorder_sound_solo_play((MMHandleType)hcamcorder, _MMCAMCORDER_FILEPATH_CAPTURE_EXT_01_SND, FALSE);
				break;
			case MM_CAM_CAPTURE_SOUND_EXT_02:
				_mmcamcorder_sound_solo_play((MMHandleType)hcamcorder, _MMCAMCORDER_FILEPATH_CAPTURE_EXT_02_SND, FALSE);
				break;
			case MM_CAM_CAPTURE_SOUND_DEFAULT:
			default:
...
int _mmcamcorder_image_cmd_preview_start(MMHandleType handle)
{
	...
	if (!sc->bencbin_capture) {
		_mmcam_dbg_log("Preview start : No need to set new caps.");
		...
		if (current_state == MM_CAMCORDER_STATE_CAPTURING) {
			...
			if (attr) {
				...
				if (_mmcamcorder_check_supported_attribute(handle, MM_CAM_CAMERA_AF_SCAN_RANGE)) {
					mmf_attribute_set_modified(&(attr->items[MM_CAM_CAMERA_AF_SCAN_RANGE]));
					if (mmf_attrs_commit((MMHandleType)attr) == -1) {
						_mmcam_dbg_err("set af scan range error");
					} else {
						_mmcam_dbg_log("set af scan range success");
					}
				} else {
					_mmcam_dbg_log("af scan range is not supported");
				}
			} else {
	...
	if (state == GST_STATE_PLAYING) {
		...
		if (hcamcorder->shutter_sound_policy == VCONFKEY_CAMERA_SHUTTER_SOUND_POLICY_OFF &&
			...
			/* get volume level */
			__ta__("        mm_sound_volume_get_value:VOLUME_TYPE_SYSTEM",
			...
			/* register changed_cb */
			__ta__("        vconf_notify_key_changed:SOUND_STATUS_BOOL",
			vconf_notify_key_changed(VCONFKEY_SETAPPL_SOUND_STATUS_BOOL, __sound_status_changed_cb, hcamcorder);
			);
			__ta__("        mm_sound_volume_add_callback:VOLUME_TYPE_SYSTEM",
...
static void __mmcamcorder_image_capture_cb(GstElement *element, GstBuffer *buffer1, GstBuffer *buffer2, GstBuffer *buffer3, gpointer u_data)
{
	...
	int resize_input_format = MM_PIXEL_FORMAT_NV12;
	unsigned char *resize_input_data = NULL;
	...
	if (buffer3 && GST_BUFFER_DATA(buffer3) && GST_BUFFER_SIZE(buffer3) != 0) {
		...
		_mmcam_dbg_log("resize main image [%dx%d] for screennail image [%dx%d]",
		               dest.width, dest.height, preview_width, preview_height);

		if (dest.format == MM_PIXEL_FORMAT_YUYV &&
		    _mmcamcorder_convert_YUYVto422P(GST_BUFFER_DATA(buffer1), &resize_input_data,
		                                    dest.width, dest.height, GST_BUFFER_SIZE(buffer1))) {
			resize_input_format = MM_PIXEL_FORMAT_422P;
		} else {
			resize_input_data = GST_BUFFER_DATA(buffer1);
			resize_input_format = dest.format;
		}

		if (dest.width > preview_width) {
			_mmcam_dbg_log("resize_input_data %p, buffer1 data %p, resize_input_format %d",
			               resize_input_data, GST_BUFFER_DATA(buffer1), resize_input_format);

			__ta__("                _mmcamcorder_resize_frame:SCREENNAIL",
			if (!_mmcamcorder_resize_frame(resize_input_data, dest.width, dest.height,
			                               GST_BUFFER_SIZE(buffer1), resize_input_format,
			                               &scrnl_data, &preview_width, &preview_height, &scrnl_length)) {
				_mmcam_dbg_warn("_mmcamcorder_resize_frame failed. skip screennail making...");
			} else {
				_mmcam_dbg_log("screennail resize done - %p", scrnl_data);
				scrnail.data = scrnl_data;
				scrnail.format = resize_input_format;
				scrnail.width = preview_width;
				scrnail.height = preview_height;
				scrnail.length = scrnl_length;
			}
			);

			if (resize_input_data &&
			    resize_input_data != GST_BUFFER_DATA(buffer1)) {
				free(resize_input_data);
				resize_input_data = NULL;
			}
		} else {
			_mmcam_dbg_log("no need to resize for screennail image %dx%d",
			               dest.width, dest.height);

			scrnail.data = resize_input_data;
			scrnail.format = resize_input_format;
			scrnail.width = dest.width;
			scrnail.height = dest.height;
			scrnail.length = GST_BUFFER_SIZE(buffer1);
		}
	} else {
	...
	if (scrnail.data) {
		...
		_mmcam_dbg_log("Screennail set attribute data %p, size %d", &scrnail, sizeof(scrnail));
	} else {
	...
	if (buffer2 && GST_BUFFER_DATA(buffer2) && (GST_BUFFER_SIZE(buffer2) != 0)) {
		...
		if (thumb.data == NULL) {
			if (scrnail.data && scrnail.length != 0) {
		...
		if (encode_src.data) {
			...
			if (encode_src.width > THUMBNAIL_WIDTH) {
				...
				if (0) {
#endif
					__ta__("                _mmcamcorder_downscale_UYVYorYUYV:THUMBNAIL",
					...
					thumb_length = (thumb_width * thumb_height) << 1;
				} else {
					__ta__("                _mmcamcorder_resize_frame:THUMBNAIL",
			...
			if (thumb_raw_data) {
				int encode_input_format = encode_src.format;
				unsigned char *encode_input_data = NULL;

				if (encode_src.format == MM_PIXEL_FORMAT_422P &&
				    _mmcamcorder_convert_422PtoYUYV(thumb_raw_data, &encode_input_data,
				                                    thumb_width, thumb_height, thumb_length)) {
					encode_input_format = MM_PIXEL_FORMAT_YUYV;
				} else {
					encode_input_format = encode_src.format;
					encode_input_data = thumb_raw_data;
				}

				_mmcam_dbg_log("encode_input_data %p, thumb_raw_data %p, encode_input_format %d",
				               encode_input_data, thumb_raw_data, encode_input_format);

				__ta__("                _mmcamcorder_encode_jpeg:THUMBNAIL",
				ret = _mmcamcorder_encode_jpeg(encode_input_data, thumb_width, thumb_height,
				                               encode_input_format, thumb_length, THUMBNAIL_JPEG_QUALITY,
				                               (void **)&internal_thumb_data, &internal_thumb_length, JPEG_ENCODER_HARDWARE);
				);
				if (ret) {
				...
				if (thumb_raw_data != encode_src.data) {
					free(thumb_raw_data);
					if (thumb_raw_data == encode_input_data)
						encode_input_data = NULL;
					thumb_raw_data = NULL;
					_mmcam_dbg_log("release thumb_raw_data");
				}

				if (encode_input_data &&
				    encode_input_data != thumb_raw_data) {
					free(encode_input_data);
					encode_input_data = NULL;
					_mmcam_dbg_log("release encode_input_data");
				}
			} else {
	...
	if (ret != MM_ERROR_NONE) {
		...
		if(!provide_exif) {
			...
			__ta__("                    __mmcamcorder_update_exif_info",
			ret = __mmcamcorder_update_exif_info((MMHandleType)hcamcorder);
			);
		}
...
error:
	...
	if (scrnl_data) {
		_mmcam_dbg_log("release resized screenail data %p", scrnl_data);
		free(scrnl_data);
		scrnl_data = NULL;
	}
	if (resize_input_data) {
		_mmcam_dbg_log("release resize_input_data %p", resize_input_data);
		free(resize_input_data);
		resize_input_data = NULL;
	}
...
static ExifData *
__mmcamcorder_update_exif_orientation(MMHandleType handle, ExifData *ed)
{
	...
	/* calibrate orientation info */
	switch (sc->videosrc_rotate) {
	case MM_VIDEO_INPUT_ROTATION_90:
		switch (value) {
		case MM_CAMCORDER_TAG_ORT_0R_VT_0C_VL:/*1*/
			value = MM_CAMCORDER_TAG_ORT_0R_VL_0C_VB;
			break;
		case MM_CAMCORDER_TAG_ORT_0R_VR_0C_VT:/*6*/
			value = MM_CAMCORDER_TAG_ORT_0R_VT_0C_VL;
			break;
		case MM_CAMCORDER_TAG_ORT_0R_VB_0C_VR:/*3*/
			value = MM_CAMCORDER_TAG_ORT_0R_VR_0C_VT;
			break;
		case MM_CAMCORDER_TAG_ORT_0R_VL_0C_VB:/*8*/
			value = MM_CAMCORDER_TAG_ORT_0R_VB_0C_VR;
			break;
		}
		break;
	...
	case MM_VIDEO_INPUT_ROTATION_270:
		...
		break;
	default:
		_mmcam_dbg_log("no camera rotation");
		break;
	}
	...
	ret = mm_exif_set_add_entry(ed, EXIF_IFD_0, EXIF_TAG_ORIENTATION,
	                            EXIF_FORMAT_SHORT, 1, (unsigned char*)&eshort);
	if( ret != MM_ERROR_NONE ) {
...
static ExifData *
__mmcamcorder_update_exif_make(MMHandleType handle, ExifData *ed)
{
	int ret = MM_ERROR_NONE;
	...
	if (make) {
		...
		ret = mm_exif_set_add_entry(ed, EXIF_IFD_0, EXIF_TAG_MAKE,
		                            EXIF_FORMAT_ASCII, strlen(make)+1, (unsigned char *)make);
		free(make);
		if (ret != MM_ERROR_NONE) {
...
static ExifData *
__mmcamcorder_update_exif_software(MMHandleType handle, ExifData *ed)
{
	int ret = MM_ERROR_NONE;
	...
	if (hcamcorder->software_version) {
		ret = mm_exif_set_add_entry(ed, EXIF_IFD_0, EXIF_TAG_SOFTWARE, EXIF_FORMAT_ASCII,
		                            strlen(hcamcorder->software_version)+1, (unsigned char *)hcamcorder->software_version);
		if (ret != MM_ERROR_NONE) {
...
static ExifData *
__mmcamcorder_update_exif_model(MMHandleType handle, ExifData *ed)
{
	int ret = MM_ERROR_NONE;
	mmf_camcorder_t *hcamcorder = (mmf_camcorder_t *)handle;
	...
	if (hcamcorder->model_name) {
		ret = mm_exif_set_add_entry(ed, EXIF_IFD_0, EXIF_TAG_MODEL, EXIF_FORMAT_ASCII,
		                            strlen(hcamcorder->model_name)+1, (unsigned char *)hcamcorder->model_name);
		if (ret != MM_ERROR_NONE) {
...
static ExifData *
__mmcamcorder_update_exif_gps(MMHandleType handle, ExifData *ed)
{
	int ret = MM_ERROR_NONE;
	int gps_enable = TRUE;
	char *err_name = NULL;
...
int __mmcamcorder_update_exif_info(MMHandleType handle)
{
	int ret = MM_ERROR_NONE;
	...
	__ta__("                        __mmcamcorder_update_exif_make",
	__mmcamcorder_update_exif_make(handle, ed);
	);
	__ta__("                        __mmcamcorder_update_exif_software_version",
	__mmcamcorder_update_exif_software(handle, ed);
	);
	__ta__("                        __mmcamcorder_update_exif_model",
	__mmcamcorder_update_exif_model(handle, ed);
	);
	__ta__("                        __mmcamcorder_update_exif_orientation",
	__mmcamcorder_update_exif_orientation(handle, ed);
	);
	__ta__("                        __mmcamcorder_update_exif_gps",
	__mmcamcorder_update_exif_gps(handle, ed);
	);
	__ta__("                        mm_exif_set_exif_to_info",
...
int __mmcamcorder_set_exif_basic_info(MMHandleType handle, int image_width, int image_height)
{
...
#ifdef WRITE_EXIF_DEBUGGING_INFO
	if (avsys_exif_info.p_embedded_data) {
		((unsigned char *)avsys_exif_info.p_embedded_data)[EXIF_DEBUGGING_INFO_LENGTH] = '\0';
		_mmcam_dbg_warn("AF + AWB + AE + IPC debugging info : [%p]", avsys_exif_info.p_embedded_data);
		ret = mm_exif_set_add_entry(ed, EXIF_IFD_EXIF, EXIF_TAG_USER_COMMENT,
		                            EXIF_FORMAT_ASCII, EXIF_DEBUGGING_INFO_LENGTH, (unsigned char *)avsys_exif_info.p_embedded_data);
	} else {
		_mmcam_dbg_warn("AF/AWB debugging info is NULL");
#endif /* WRITE_EXIF_DEBUGGING_INFO */
	...
	if (avsys_exif_info.aperture_in_APEX) {
		...
		if (b) {
			...
			rData.denominator = 100;	// camera source plugin should be multiple *100 for using below decimal point
			exif_set_rational(b, exif_data_get_byte_order(ed), rData);
