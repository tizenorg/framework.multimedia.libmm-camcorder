		...
		switch (buf) {
			...
			case 'H' : // Setting > Hybrid mode
				...
				if (index < 0 || index > 1) {
					...
					camcordertest_set_attr_int(MMCAM_CAMERA_HYBRID_MODE, index);
				}
			...
			case 'C' : //  Setting > Capture sound
				g_print("* Capture sound!\n");
				camcordertest_get_attr_valid_intrange(MMCAM_CAPTURE_SOUND, &min, &max);

				if( min > max ) {
					g_print("Not supported !! \n");
				} else {
					flush_stdin();
					g_print("\n Select Capture sound (%d ~ %d)\n", min, max);
					for( i = min ; i <= max ; i++ ) {
						g_print( "%d. %s\n", i, capture_sound[i] );
					}
					err = scanf("%d",&index);
					bret = camcordertest_set_attr_int(MMCAM_CAPTURE_SOUND, index);
				}
				break;
...
static gboolean init(int type)
{
	...
	int display_rotation = MM_DISPLAY_ROTATION_NONE;
	...
	*=================================================================================*/
	if (type == MODE_VIDEO_CAPTURE) {
		mm_camcorder_get_attributes((MMHandleType)cam_handle, NULL,
		                            ...
		                            MMCAM_RECOMMEND_DISPLAY_ROTATION, &display_rotation,
		                            NULL);
		...
		err = mm_camcorder_set_attributes( (MMHandleType)cam_handle, &err_attr_name,
		                                   ...
		                                   //MMCAM_VIDEO_WIDTH, 1280,
		                                   //MMCAM_VIDEO_HEIGHT, 720,
		                                   MMCAM_CAMERA_HYBRID_MODE, FALSE,
		                                   ...
		                                   MMCAM_DISPLAY_ROTATION, display_rotation,
...
static gboolean mode_change()
{
	...
	g_timer_reset(timer);
	mm_camcorder_set_attributes(hcamcorder->camcorder, NULL, MMCAM_FILTER_COLOR_TONE, color_tone, NULL);
